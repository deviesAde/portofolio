...
const FloatingDockDesktop = ({ items, className }) => {
  ...
  return (
    <motion.div
      ...
      onMouseLeave={() => mouseX.set(Infinity)}
      className={cn(
        "mx-auto hidden md:flex h-12 gap-2 items-end rounded-xl bg-gray-50 dark:bg-neutral-900 px-3 pb-2", // Sebelumnya h-16, gap-4, px-4, pb-3
        className
      )}
    >
      {items.map((item) => (
...
function IconContainer({ mouseX, title, icon, href }) {
 ...
 let widthTransform = useTransform(distance, [-150, 0, 150], [30, 60, 30]); // Sebelumnya 40, 80, 40
 let heightTransform = useTransform(distance, [-150, 0, 150], [30, 60, 30]);

 let widthTransformIcon = useTransform(distance, [-150, 0, 150], [15, 30, 15]); // Sebelumnya 20, 40, 20
